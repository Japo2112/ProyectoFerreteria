...
-- (Ignora el error ORA-00600 si vuelve a salir, solo queremos borrarlos)

-- Borramos la tabla USUARIO
DROP TABLE USUARIO CASCADE CONSTRAINTS;

-- La volvemos a crear, esta vez con el hash NOT NULL
CREATE TABLE USUARIO (
  id_usuario       NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  nombre           VARCHAR2(120) NOT NULL,
  username         VARCHAR2(80) UNIQUE NOT NULL,
  password_hash    VARCHAR2(300) NOT NULL, -- <<<--- ¡CORREGIDO Y REQUERIDO!
  activo           CHAR(1) DEFAULT 'S' CHECK (activo IN ('S','N')),
  rol_app_id       NUMBER, 
  last_login       TIMESTAMP,
  CONSTRAINT fk_usuario_rol FOREIGN KEY (rol_app_id) REFERENCES ROLES_APLICACION(rol_app_id)
...
-- Borramos el trigger de login de SYS, ya no lo necesitamos
-- (Debes conectarte como SYS en XEPDB1 para esto)
DROP TRIGGER trg_app_actualizar_last_login;
